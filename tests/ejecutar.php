...
$bloqueado = false;
for ($i = 0; $i < 8; $i++) {
    $bloqueado = $bloqueado || App\Middleware\RateLimit::demasiadosEnVentana('prueba_asistente', 8, 30);
}
Pruebas::afirmar($bloqueado === false, 'Ocho preguntas seguidas pasan sin estorbo');
Pruebas::afirmar(App\Middleware\RateLimit::demasiadosEnVentana('prueba_asistente', 8, 30) === true,
    'La novena se frena');
...
// =====================================================================
Pruebas::grupo('23 · La lista de suscriptores');
// =====================================================================

use App\Models\Subscriber;

$correoPrueba = 'lector-' . $sufijo . '@ejemplo.test';
$consentido   = 'Texto exacto que aceptó, prueba ' . $sufijo;

// Un alta guarda el texto aceptado. Sin eso no hay consentimiento que
// demostrar, solo una dirección en una tabla.
$token = Subscriber::alta($correoPrueba, ['resumen_semanal'], $consentido);
Pruebas::afirmar(preg_match('/^[0-9a-f]{48}$/', $token) === 1, 'El alta devuelve un token de 48 caracteres');

$fila = Database::first('SELECT * FROM subscribers WHERE email = :e', ['e' => $correoPrueba]);
Pruebas::iguales($consentido, $fila['consent_text'], 'Se guarda el texto exacto que la persona aceptó');
Pruebas::iguales(null, $fila['confirmed_at'], 'Nadie nace confirmado: el alta empieza sin confirmar');

// Nadie sin confirmar puede recibir un correo. Esta es la prueba que
// impide escribirle a quien no dijo que sí.
$destinos = array_column(Subscriber::destinatarios('resumen_semanal'), 'email');
Pruebas::afirmar(!in_array($correoPrueba, $destinos, true),
    'Un alta sin confirmar NO entra en la lista de envío');

Subscriber::confirmar($token);
$destinos = array_column(Subscriber::destinatarios('resumen_semanal'), 'email');
Pruebas::afirmar(in_array($correoPrueba, $destinos, true),
    'Una vez confirmado, sí entra en la lista de envío');

// Confirmar dos veces no puede romper nada ni mover la fecha.
$antes = Database::value('SELECT confirmed_at FROM subscribers WHERE email = :e', ['e' => $correoPrueba]);
Subscriber::confirmar($token);
$despues = Database::value('SELECT confirmed_at FROM subscribers WHERE email = :e', ['e' => $correoPrueba]);
Pruebas::iguales($antes, $despues, 'Confirmar dos veces no cambia la fecha de confirmación');

// La baja tiene que funcionar siempre y a la primera.
Pruebas::afirmar(Subscriber::baja($token), 'La baja acepta el mismo token del alta');
$destinos = array_column(Subscriber::destinatarios('resumen_semanal'), 'email');
Pruebas::afirmar(!in_array($correoPrueba, $destinos, true),
    'Quien se da de baja desaparece de la lista de envío inmediatamente');
Pruebas::afirmar(
    Database::value('SELECT id FROM subscribers WHERE email = :e', ['e' => $correoPrueba]) !== null,
    'La baja no borra el registro: hay que poder probar que la pidió'
);
Pruebas::afirmar(Subscriber::baja($token), 'Darse de baja dos veces tampoco es un error');

// El token sobrevive a la confirmación. Si se borrara, el enlace de baja
// de un correo ya enviado dejaría de funcionar.
$tokenAhora = Database::value('SELECT confirm_token FROM subscribers WHERE email = :e', ['e' => $correoPrueba]);
Pruebas::iguales($token, $tokenAhora, 'El token no se borra al confirmar: sigue sirviendo para la baja');

// Volver a apuntarse reactiva, no duplica.
$token2 = Subscriber::alta($correoPrueba, ['resumen_semanal', 'avisos_live'], $consentido);
Pruebas::iguales($token, $token2, 'Volver a apuntarse conserva el token original');
Pruebas::iguales(1,
    (int) Database::value('SELECT COUNT(*) FROM subscribers WHERE email = :e', ['e' => $correoPrueba]),
    'Volver a apuntarse no crea una segunda fila');
Pruebas::iguales(null,
    Database::value('SELECT unsubscribed_at FROM subscribers WHERE email = :e', ['e' => $correoPrueba]),
    'Volver a apuntarse levanta la baja');

// Un token inventado no puede dar de baja ni confirmar a nadie.
foreach (['', 'no-es-un-token', str_repeat('z', 48), str_repeat('a', 47)] as $malo) {
    Pruebas::afirmar(Subscriber::confirmar($malo) === false, 'Un token inválido no confirma a nadie');
    Pruebas::afirmar(Subscriber::baja($malo) === false, 'Un token inválido no da de baja a nadie');
}

// Los intereses que llegan del formulario son datos de fuera: solo pasan
// los que la base admite, y nunca se queda vacío.
Pruebas::iguales(['resumen_semanal'], Subscriber::limpiarPropositos([]), 'Sin intereses se aplica el resumen');
Pruebas::iguales(['resumen_semanal'], Subscriber::limpiarPropositos(['inventado', 'otro']), 'Un interés inventado se descarta');
Pruebas::iguales(['avisos_live'], Subscriber::limpiarPropositos(['avisos_live', 'avisos_live']), 'Un interés repetido no se duplica');
Pruebas::afirmar(Subscriber::limpiarPropositos([['array'], 5, null]) === ['resumen_semanal'],
    'Un interés que ni siquiera es texto no rompe nada');

// Un propósito que no existe no puede devolver a toda la lista.
Pruebas::iguales([], Subscriber::destinatarios('inventado'), 'Un propósito inexistente no devuelve destinatarios');
