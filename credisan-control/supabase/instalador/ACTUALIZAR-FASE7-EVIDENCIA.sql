-- =====================================================================
--
--   CREDISAN CONTROL · ACTUALIZACIÓN · VER LA EVIDENCIA FOTOGRÁFICA
--
-- =====================================================================
--
--   Su base ya está instalada. Esto NO BORRA NADA y no crea tablas.
--
--   Cierra un hueco de fondo: desde la Fase 3 cada marcación guarda una
--   fotografía del momento —la prueba de que quien marcó es la persona
--   del PIN— pero NADIE PODÍA MIRARLA. Una prueba que no se consulta no
--   prueba nada.
--
--   Quién podrá verla: dirección, administración y el jefe operativo,
--   cada uno en su sede. Y MIRAR DEJA RASTRO: queda escrito quién miró
--   la fotografía de quién y cuándo.
--
--   1. SQL Editor → New query
--   2. Pegue todo este archivo
--   3. Run  (acepte el aviso: es por el `revoke`)
--
-- =====================================================================
...
begin
  ...
  if to_regproc('public.edge_offline') is null then
    raise exception 'Falta la Fase 6 en esta base. Ejecute antes ACTUALIZAR-FASE6.sql.';
  end if;
end $$;
...
-- =====================================================================
do $$
declare v_fn int; v_abierta int; v_emp int; v_ev int;
begin
  select count(*) into v_fn from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname in ('evidencia_de','edge_evidencia_ruta');

  select count(*) into v_abierta from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'edge_evidencia_ruta'
     and (has_function_privilege('authenticated', p.oid, 'execute')
       or has_function_privilege('anon', p.oid, 'execute'));

  select count(*) into v_emp from employees;
  select count(*) into v_ev  from attendance_evidence;

  raise notice '';
  raise notice '  CREDISAN CONTROL — consulta de evidencia activada';
  raise notice '  ------------------------------------------------';
  raise notice '  Funciones nuevas ........... % de 2', v_fn;
  raise notice '  Trabajadores (intactos) .... %', v_emp;
  raise notice '  Fotografías guardadas ...... %', v_ev;
  raise notice '';
  ...
  if v_abierta > 0 then
    raise exception 'ATENCION: la ruta quedó accesible sin pasar por el registro. No debe.';
  end if;

  raise notice '  Toda consulta queda registrada: comprobado.';
  raise notice '  En el panel, las marcaciones con foto se abren pulsándolas.';
  raise notice '';
end $$;
