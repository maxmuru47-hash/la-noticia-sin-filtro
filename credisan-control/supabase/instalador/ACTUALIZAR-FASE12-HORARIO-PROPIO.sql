-- =====================================================================
--
--   CREDISAN CONTROL · FASE 12 · horario propio de un trabajador
--
-- =====================================================================
--
--   Su base ya está instalada. Esto NO BORRA NADA: añade cuatro
--   funciones. Ninguna tabla cambia, y el motor de medición tampoco:
--   ya sabía hacer esto desde la fase 1.
--
--   PARA QUÉ SIRVE
--   --------------
--   Hay gente que no hace el horario de su sede —el vigilante que entra
--   más tarde, quien hace jornada corrida donde los demás la hacen
--   partida—. Medirla contra el horario general la convierte en
--   impuntual todos los días por una diferencia que la empresa aprobó.
--
--   Desde ahora, la ADMINISTRADORA DE CADA SEDE puede darle a los suyos
--   su propio horario desde el panel: Personal → el trabajador →
--   «Horario». El jefe operativo no puede, y la administradora de otra
--   sede tampoco.
--
--   LO QUE NO SE TOCA: el pasado. Cambiar o quitar un horario rige
--   desde HOY. Los días ya cerrados se quedan como se midieron.
--
...
do $$
begin
  if to_regprocedure('public.dar_horario_propio(uuid, jsonb)') is null
     or to_regprocedure('public.quitar_horario_propio(uuid)') is null
     or to_regprocedure('public.horario_de_trabajador(uuid)') is null
     or to_regprocedure('public.horarios_propios(uuid)') is null then
    raise exception 'La fase 12 no quedó completa: falta alguna función.';
  end if;
  raise notice 'FASE 12 lista. Panel → Personal → el trabajador → «Horario».';
end $$;
