-- =====================================================================
--
--   CREDISAN CONTROL · ACTUALIZACIÓN A LA FASE 6
--   Marcar sin conexión
--
-- =====================================================================
--
...
--   ninguna tabla.
--
--   Añade la puerta por la que entran las marcaciones que se hicieron sin
--   internet, y CORRIGE UN FALLO del motor que sólo se ve cuando se
--   sincroniza de verdad (se explica dentro, a mitad del archivo).
--
--   1. SQL Editor → New query
--   2. Pegue todo este archivo
--   3. Run  (acepte el aviso de operaciones destructivas: es por los
--      `revoke`, que son los que cierran la puerta)
--
--   DESPUÉS DE ESTO FALTAN DOS PASOS, sin los cuales el modo sin conexión
--   no se activa. Están en docs/FASE6_SIN_CONEXION.md:
--
--     · generar el par de llaves y guardar la privada como secreto
--     · pegar la pública en config/env.js
--
-- =====================================================================
...
begin
  ...
  if to_regproc('public.calcular_cierre_semana') is null then
    raise exception
      'Falta la Fase 5 en esta base. Ejecute antes ACTUALIZAR-FASE5.sql.';
  end if;
  if to_regproc('public.edge_offline') is not null then
    raise notice 'La Fase 6 ya estaba aplicada. Se actualizarán las funciones.';
  end if;
end $$;
...
-- =====================================================================
do $$
declare v_fn int; v_abierta int; v_emp int; v_mar int;
begin
  select count(*) into v_fn from pg_proc p
    join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public'
     and p.proname in ('edge_offline','sincronizacion','marcaciones_offline');

  -- La puerta del servidor tiene que seguir cerrada para el panel
  select count(*) into v_abierta
    from pg_proc p
    join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'edge_offline'
  ...
  raise notice '';
  raise notice '  CREDISAN CONTROL — Fase 6 aplicada';
  raise notice '  ----------------------------------';
  raise notice '  Funciones nuevas ........... % de 3', v_fn;
  ...
  if v_abierta > 0 then
    raise exception 'ATENCION: edge_offline quedó accesible desde el panel. No debe.';
  end if;

  raise notice '  La puerta del servidor sigue cerrada al panel: comprobado.';
  raise notice '';
  raise notice '  FALTAN DOS PASOS para que el modo sin conexión funcione:';
  raise notice '   1. node supabase/functions/generar-llaves.mjs';
  raise notice '   2. La privada como secreto CREDISAN_OFFLINE_PRIVATE_KEY,';
  raise notice '      la pública en config/env.js como offlinePublicKey.';
  raise notice '';
  raise notice '  Sin eso el terminal marca igual, pero sólo con internet.';
  raise notice '';
end $$;
