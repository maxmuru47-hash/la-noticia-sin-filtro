-- =====================================================================
--
--   CREDISAN CONTROL · ACTUALIZACIÓN A LA FASE 4
--   Tableros: la operación del día, el período y las novedades
--
-- =====================================================================
--
--   Su base ya está instalada y con datos. Esto NO TOCA NI UN DATO:
--   no crea tablas, no borra nada, no modifica su personal, sus
--   horarios ni sus marcaciones. Sólo añade siete funciones de
--   consulta para que el panel tenga qué mostrar.
--
...
begin
  ...
  if to_regproc('public.marcaciones_hoy') is null then
    raise exception
      'Falta la Fase 3 en esta base. Ejecute antes ACTUALIZAR-FASE3.sql.';
  end if;
  if to_regproc('public.tablero_hoy') is not null then
    raise notice 'La Fase 4 ya estaba aplicada. Se actualizarán las funciones.';
  end if;
end $$;
...
-- =====================================================================
do $$
declare v_fn int; v_emp int; v_mar int;
begin
  select count(*) into v_fn from pg_proc p
    join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public'
     and p.proname in ('tablero_hoy','tablero_periodo','ranking_puntualidad',
                       'registrar_novedad','resolver_novedad','novedades','recalcular_rango');

  select count(*) into v_emp from employees;
  select count(*) into v_mar from attendance_events;

  raise notice '';
  raise notice '  CREDISAN CONTROL — Fase 4 aplicada';
  raise notice '  ----------------------------------';
  raise notice '  Funciones de tablero ....... % de 7', v_fn;
  raise notice '  Trabajadores (intactos) .... %', v_emp;
  raise notice '  Marcaciones (intactas) ..... %', v_mar;
  raise notice '';

  if v_fn < 7 then
    raise exception 'ATENCION: faltan funciones de tablero. La actualización no se completó.';
  end if;

  raise notice '  Listo. Entre al panel y verá la pestaña «Hoy» con datos.';
  raise notice '';
end $$;
