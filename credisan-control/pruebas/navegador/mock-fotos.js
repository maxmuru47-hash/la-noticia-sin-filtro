...
(function () {
  ...
  const TODOS_EMP = [
    { id:'e1', branch_id:'b-mcb', internal_code:'MCB-001', first_name:'Ana',  last_name:'Pérez',
      national_id:'V-12345678', position:'Cajera', hired_on:'2026-01-15', is_active:true,  pin_updated_at:null, photo_path:'b-mcb/e1.jpg' },
  ...
  const almacenamiento = {
    from(bucket) {
      return {
        upload: (ruta, cuerpo, opts) => {
          window.__SUBIDAS.push({ bucket, ruta, bytes: cuerpo.size, tipo: cuerpo.type, opts });
          return Promise.resolve({ data: { path: ruta }, error: null });
        },
        createSignedUrl: (ruta) => Promise.resolve({
          // Un PNG rojo de 1×1, que carga de verdad en el navegador
          data: { signedUrl: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==' },
          error: null
        })
      };
    }
